...
import com.coolu.nokelock.bike.bean.ImageTitleBean;
...
public class ImageSlideshow extends FrameLayout {
    ...
    // 设置小圆点的大小
    public void setDotWidth(int dotSize) {
        this.dotWidth = dotSize;
    }
    ...
    private void setIndicator() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(dotWidth, dotHeight); //宽高
    ...
    private void setViewList(List<ImageTitleBean> imageTitleBeanList) {
        ...
        for (int i = 0; i < count + 2; i++) {
            ...
            if (i == 0) {// 将最前面一页设置成本来最后的那页
                //Picasso.with(context).load(imageTitleBeanList.get(count - 1).getImageUrl()).resize(100,200).centerCrop().transform(new RoundedCornersTransformation(20,0)).into(ivImage);
                Log.e("aa",imageTitleBeanList.get(count-1).getImageUrl());
                Picasso.with(context).load(imageTitleBeanList.get(count - 1).getImageUrl()).fit().transform(new RoundedCornersTransformation(20,0)).into(ivImage);
                tvTitle.setText(imageTitleBeanList.get(count - 1).getTitle());
            } else if (i == count + 1) {// 将最后面一页设置成本来最前的那页
                Picasso.with(context).load(imageTitleBeanList.get(0).getImageUrl()).fit().transform(new RoundedCornersTransformation(20,0)).into(ivImage);
                tvTitle.setText(imageTitleBeanList.get(0).getTitle());
            } else {
                Picasso.with(context).
                        load(imageTitleBeanList.get(i - 1).getImageUrl()).fit().transform(new RoundedCornersTransformation(20,0)).into(ivImage);
