package com.coolu.nokelock.bike.overlay;
	...
	public void addToMap() {
        ...
        try {
          ...
          // mPolylineOptions.add(startPoint);
			ll = AMapServicesUtil.convertToLatLng(walkPaths.get(0)
					.getPolyline().get(0));
            for (int i = 0; i < walkPaths.size(); i++) {
				...
				//addWalkStationMarkers(walkStep, latLng); //去掉路线上沿途标志点
                addWalkPolyLines(walkStep);
               
            }
           //mPolylineOptions.add(endPoint);
          addStartAndEndMarker(ll);
